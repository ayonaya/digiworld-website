...
const PAYPAL_API_BASE_URL = 'https://api-m.paypal.com'; // Use sandbox URL for testing if needed
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (paypalStatus === 'COMPLETED') {
            ...
            // --- FIX: Loop through the cart and get a key for each item ---
            for (const item of cart) {
                // Get 'item.quantity' number of keys for this 'item.id'
        ...
        const orderDocRef = db.collection('orders').doc(orderID);
        // Avoid crashing if the document doesn't exist yet on an early error
        if (orderDocRef) {
            await orderDocRef.update({ status: 'error', errorDetails: error.message }).catch(e => console.error("Failed to update order status on error:", e));
        }
        return {
            statusCode: 500,
